...
Produce a coordination plan for splitting the active framework-development task across multiple agents. This
command **plans only** — it does not create branches/worktrees or spawn agents itself; a human or the calling
session acts on the plan.
...
For each file in scope, assign exactly one writer for the wave — this repo's default ownership table is
`docs/how-to/multi-agent-collaboration.md` §6 (`AGENTS.md`/`CLAUDE.md`, `.workspace/**`, `harness-manifest.json`,
`HARNESS-VERSION`, and `FRAMEWORK-CHANGELOG.md` are Coordinator-only by default). Never assign the same file to two
agents in one wave.
...
`.workspace/**`, `AGENTS.md`, `CLAUDE.md`, `harness-manifest.json`, `HARNESS-VERSION`, `FRAMEWORK-CHANGELOG.md`, and
any generated/derived artifact are Coordinator-only in every wave — no agent contract may include them.

### 8. Define Integration Order and Validation

State the order agents are merged in and the exact full validation command:

```bash
pnpm validate   # check-sync + typecheck + lint + test
```

Note the working directory and any environment flags a language pack's own `validate.sh`/`.ps1` needs, if the task
touches `language-packs/**`.
